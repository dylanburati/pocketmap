...
public class ShortPocketMap extends AbstractMap<String, Short> implements Cloneable {
  ...
  private boolean containsEntry(Map.Entry<?, ?> e) {
    ...
    return idx >= 0 && this.values[idx] == (Short) value;
  }
  ...
  private Short putImpl(String key, Short value, boolean shouldReplace) {
  ...
  public boolean remove(Object key, Object value) {
    ...
    byte[] keyContent = ((String) key).getBytes(StandardCharsets.UTF_8);
    int idx = this.readIndex(keyContent);
    if (idx >= 0 && this.values[idx] == (Short) value) {
      // removeByIndex condition upheld: readIndex only returns a valid index if (keys[idx] & ALIVE_FLAG) == ALIVE_FLAG
      this.removeByIndex(idx);
      return true;
    }
  ...
  private Short computeImpl(String key, BiFunction<? super String, ? super Short, ? extends Short> remappingFunction, boolean shouldInsert, boolean shouldReplace) {
  ...
  @Override
  public void putAll(Map<? extends String, ? extends Short> m) {
    for (Map.Entry<? extends String, ? extends Short> e : m.entrySet()) {
