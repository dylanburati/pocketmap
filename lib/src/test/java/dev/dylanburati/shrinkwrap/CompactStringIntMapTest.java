...
class CompactStringIntMapTest {
  @Test void test_create_capacity_zero() {
  ...
  void testLotsOfInsertions(int initialCapacity) {
    ...
    /* template! \(if .isObject then "Function<V, String> toString = (v) -> v.toString();" else "\(.primitive | pascal)Function<String> toString = \(.boxed)::toString;" end) */
    ...
    for (int loop = 0; loop < 10; loop++) {
      ...
      for (int i = 1; i < count; i++) {
        assertNull(m.put(toString.apply(i), i));

        for (int j = 1; j <= i; j++) {
          assertEquals(m.get(toString.apply(j)), j);
        }
        for (int j = i+1; j < count; j++) {
          assertNull(m.get(toString.apply(j)));
        }
      }

      for (int i = count; i < 2*count; i++) {
        assertNull(m.get(toString.apply(i)));
      }

      // remove forwards
      for (int i = 1; i < count; i++) {
        assertEquals(m.remove(toString.apply(i)), i);

        for (int j = 1; j <= i; j++) {
          assertFalse(m.containsKey(toString.apply(j)));
        }
        for (int j = i+1; j < count; j++) {
          assertTrue(m.containsKey(toString.apply(j)));
        }
      }

      for (int i = 1; i < count; i++) {
        assertFalse(m.containsKey(toString.apply(i)));
      }
  ...
  @Test void test_insert_overwrite() {
    /* template! CompactString\(.primitive | pascal)Map m = new CompactString\(.primitive | pascal)Map(); */
    CompactStringIntMap m = new CompactStringIntMap();
    assertNull(m.put("a", 1));
    assertEquals(m.get("a"), 1);
    assertEquals(m.put("a", 2), 1);
    assertEquals(m.get("a"), 2);
  }

  @Test void test_conflict_remove() {
    /* template! CompactString\(.primitive | pascal)Map m = new CompactString\(.primitive | pascal)Map(); */
    CompactStringIntMap m = new CompactStringIntMap();
    assertNull(m.put("a", 1));
    assertEquals(m.get("a"), 1);
    ...
    assertNull(m.put("%($", 5));
    assertEquals(m.get("a"), 1);
    assertEquals(m.get("%($"), 5);

    assertNull(m.put("?/4-AW\u0000", 9));
    assertEquals(m.get("a"), 1);
    assertEquals(m.get("%($"), 5);
    assertEquals(m.get("?/4-AW\u0000"), 9);
  ...
  @Test void test_iterate() {
    ...
    for (Entry<String, Integer> e : m.entrySet()) {
      assertEquals(e.getKey(), toString.apply(e.getValue()));
      long mask = 1L << e.getValue();
      assertEquals(observed & mask, 0L, String.format("unexpected second occurence of %d", e.getValue()));
      observed |= mask;
    }

    assertEquals(observed, 0xFFFF_FFFFL);
  }
}
/*
